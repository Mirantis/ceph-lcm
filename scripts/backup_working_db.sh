...
while getopts ":h" opt; do
  case $opt in
    h)
      echo "Backup Shrimp database on _working_ containers."
      echo ""
...
docker exec "${db_container}" bash -c "(mkdir -p /backup || true) && cd /backup && mongodump --gzip --ssl && cd / && tar cvfz /backup.tar.gz /backup && rm -rf /backup" \

...
def test_create_new_user(
    ...
    response = client_v1.post("/v1/user/", data=request)
    db_user = pymongo_connection.db.user.find({"login": request["login"]})
    ...
    assert data["data"]["login"] == request["login"]
    assert data["data"]["email"] == db_user["email"]
    assert data["data"]["email"] == request["email"]
    assert data["data"]["full_name"] == request["full_name"]
    assert data["data"]["full_name"] == db_user["full_name"]
    assert [r["id"] for r in data["data"]["roles"]] == request["role_ids"]
...
def test_create_new_user_same_data(field, email, pymongo_connection,
    ...
    assert client_v1.post("/v1/user/", data=request).status_code == 200
...
def test_update_idempotent(email, client_v1, freeze_time):
    request = {
        "login": str(uuid.uuid4()),
        "email": "{0}@example.com".format(uuid.uuid4()),
        "role_ids": [],
...
def test_2users_update_previous_data(field, email, client_v1, freeze_time):
    ...
    response1 = client_v1.post("/v1/user/", data=request1)
    ...
    another_model["data"][field] = request1[field]
...
def test_user_delete_twice(email, client_v1, freeze_time):
    request = {
        "login": str(uuid.uuid4()),
        "email": "{0}@example.com".format(uuid.uuid4()),
        "role_ids": [],
...
def test_user_delete_update(email, client_v1, freeze_time):
    request = {
        "login": str(uuid.uuid4()),
        "email": "{0}@example.com".format(uuid.uuid4()),
        "role_ids": [],
